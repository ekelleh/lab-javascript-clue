...
var collection = [roomsArray, charactersArray, weaponsArray];

function randomSelector(arr) {
    const random = Math.floor(Math.random() * arr.length);
    const rdmObj = arr[random];
    return rdmObj;
}

function pickMistery(arr) {
    var a = randomSelector(weaponsArray);
    var b = randomSelector(charactersArray);
    var c = randomSelector(roomsArray);
    let misteryEnvelope = [b, a, c];
    return misteryEnvelope;
}

function revealMistery(misteryEnvelope) {
    var result = [];
    result = `${misteryEnvelope[0].first_name} ${
